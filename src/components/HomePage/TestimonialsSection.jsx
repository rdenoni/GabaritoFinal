...
const TestimonialsSection = () => {
    ...
    return (
        <motion.section 
            style={{backgroundColor: 'var(--color-bg-test)'}} 
           ...
           <div className="max-w-3xl mx-auto">
                ...
                <div className="flex flex-col gap-6">
                    {testimonials.map((testimonial) => (
                        <motion.div key={testimonial.id} className="bg-white rounded-xl p-6 flex flex-col sm:flex-row items-center sm:items-start gap-4 border-l-8 border-[--color-accent] cursor-pointer transform transition-all duration-300 hover:-translate-y-2 hover:shadow-xl active:scale-[0.99]" whileTap={{ scale: 0.98 }}>
                            ...
                            <div className="flex-1 text-center sm:text-left">
                                ...
                                <p style={{font: 'var(--font-p-small)'}} className="text-gray-700">{`"${testimonial.quote}"`}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
